...
#define MIN_MATCH_ALLOCATION_SIZE 4096
...
void free_offsets(MatchedOffsets_t *offsets) {
    if (offsets->matchbuffer != NULL) {
        free(offsets->matchbuffer);
    }
    free(offsets);
}

MatchedOffsets_t *scan_exact(uint8_t *buffer, size_t buffer_size, uint8_t *data, size_t data_size, int increment) {
    const size_t allocation_size = buffer_size>>4 < MIN_MATCH_ALLOCATION_SIZE ? sizeof(size_t) * MIN_MATCH_ALLOCATION_SIZE : sizeof(size_t) * (buffer_size>>4);
    size_t matchbuffersize = allocation_size;
    size_t *matchbuffer = malloc(allocation_size);
    int matchbufferpos = 0;
    
    const size_t last_pos = buffer_size - data_size;
    const uint64_t value = *(uint64_t *) &(data[0]);
    int isMatched;
    for (size_t offset=0; offset<last_pos; offset += increment) {
        isMatched = ((
                        (data_size == 8 && *((uint64_t *) (buffer+offset)) == value) || // if full 64-bit is matched, it's definitely a match
                        (data_size == 4 && *((uint32_t *) (buffer+offset)) == (uint32_t) value) || // if 32-bits matched and we had <= 32 bits of data
                        (data_size == 2 && *((uint16_t *) (buffer+offset)) == (uint16_t) value) || // if 16-bits matched and we had <= 16 bits of data
                        (data_size == 1 && *((uint8_t *) (buffer+offset)) == (uint8_t) value) // if 8-bits matched and we had 8 bits of data
                    ));
        if (unlikely(isMatched)) {
            matchbuffer[matchbufferpos++] = offset;
            if (unlikely(matchbufferpos == matchbuffersize)) {
                matchbuffersize += allocation_size;
                size_t *tmpbuffer = malloc(matchbuffersize);
                memcpy(tmpbuffer, matchbuffer, matchbufferpos);
                free(matchbuffer);
                matchbuffer = tmpbuffer;
            }
        }
    }
