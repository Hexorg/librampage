...
Process_t *Process_new(int pid) {
    ...
    ProcMapFile_t *tmpmemmap = procmap_open(path);
    result->memmap = *tmpmemmap;
    free(tmpmemmap);
    return result;
}
...
void Process_free(Process_t *process) {
    fclose(process->mem);
    procmap_close(&process->memmap);
    free(process);
}

ProcessMemory_t *Process_get_bytes(Process_t *process, void *address, size_t size) {
    ProcessMemory_t *result = malloc(sizeof(ProcessMemory_t));
    result->buffer = malloc(size);
    result->address = address;
    result->buffer_length = size;
    result->bytes_read = 0;
    readMemFile(process->mem, result);
    return result;
}

void Process_free_bytes(ProcessMemory_t *bytes) {
